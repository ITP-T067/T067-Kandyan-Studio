    ...
    const fetchSalaryData = async () => {
        ...
        try {
            const response = await axios.get('/salary/salary/', {
                params: { employeeName: "B. Thissera", selectedMonth: formattedMonth },
            });
        ...
        <div>
            <div className='flex justify-start'>
                <button onClick={GoBack}>
                    <div className="flex items-center">
                        ...
                        <h1 className="text-left text-kwhite text-3xl pr-4 pb-4 ml-7 mt-4">Cashier Payroll</h1>
                    </div>
                </button>
            </div>
            <div className="flex flex-col justify-center items-center h-full text-kblack">
                <div className="w-[1000px] bg-kgray bg-opacity-30 rounded-lg p-6 shadow-lg">
                    <form className="grid grid-cols-5 gap-6 items-center justify-center" onSubmit={handleSubmit}>
                                
                                <label className="text-kwhite">Month & Year</label>
                                <div className="grid col-span-2">
                                <MonthYearPicker selectedDate={selectedMonth} handleDateChange={handleMonthChange} />
                                </div>
                                <button
                                type="submit"
                                className="bg-kgreen text-kwhite font-bold py-2 px-4 rounded transition-transform hover:scale-105 "
                            >
                                Submit
                            </button>
                        {error && <div className="text-kred font-bold mt-5">{error}</div>}
                            ...
                            {salaryData && (
                                <button onClick={generatePDF} type="button" className="bg-kblue inline-flex items-center justify-center text-kwhite font-bold py-1 px-4 rounded transition-transform hover:scale-103">
                    ...
                    {salaryData && (
                        <div className='flex flex-col items-center justify-center'>
                            <div className='px-20 py-10 mt-5 relative bg-kwhite opacity-95 rounded-lg' ref={componentPDF}>
                                <img
