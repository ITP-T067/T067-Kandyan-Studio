...
const Header = () => {
    ...
    const handleViewPaySlip = (actor) => {
        if (actor === 'Cashier') {
            window.location.href = '/cashier/cashierpayslip';
        } else if (actor === 'Creator') {
            window.location.href = '/creator/creatorpayslip';
        } else if (actor === 'Studio Operator') {
            window.location.href = '/studiooperator/studioslip';
        } else {
            console.log('You are not authorized to view pay slip');
        }
        console.log('View Pay Slip');
    }
            ...
            return (
                <nav className="navbar flex justify-between items-center px-20 top-0 left-0 bg-black py-30">
                    ...
                    <div className="flex flex-cols">
                        <button className="flex items-center space-x-2 text-sm text-kblack bg-kwhite font-bold py-2 px-4 rounded-full" onClick={() => handleViewPaySlip(userRole)}><HiCurrencyDollar />View Pay Slip</button>
